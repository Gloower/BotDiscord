...
id_do_servidor = 709975696537026680
id_cargo_atendente = 833869913956941856
token_bot = 'MTA0MjIyNTAwNjM0MTUyNTYxOQ.GAPV-9.x3EoFXn4uGqKL0ZFMBs3boObmoL7iBIxqJLnkU'
...
class Dropdown(discord.ui.Select):
  ...
  def __init__(self):
    options = [
      ...
      discord.SelectOption(value="atendimento", label="Atendimento",
                           emoji="📨"),
    ]
  ...
  async def callback(self, interaction: discord.Interaction):
    if self.values[0] == "ajuda":
      await interaction.response.send_message(
        "Se você precisar de ajuda, envie mensagem para Gloower#0001",
        ephemeral=True)
    elif self.values[0] == "atendimento":
      await interaction.response.send_message(
        "Clique abaixo para criar um ticket",
        ephemeral=True,
        view=CreateTicket())
...
async def setup(interaction: discord.Interaction):
  await interaction.response.send_message("Mensagem do painel",
                                          view=DropdownView())
